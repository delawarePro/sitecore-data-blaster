...
namespace Sitecore.DataBlaster.Read
{
    public class BulkReader
    {
        ...
        public virtual IEnumerable<ItemVersionHeader> GetDescendantVersionHeaders(SqlContext sqlContext, Guid itemId,
            string itemPath,
            Guid[] ofTemplates = null, DateTime? itemModifiedSince = null, DateTime? itemVersionModifiedSince = null)
        {
            ...
            using (var reader = sqlContext.ExecuteReader(sql, commandProcessor: cmd =>
            {
                ...
                if (itemVersionModifiedSince.HasValue)
                    cmd.Parameters.AddWithValue("@versionModifiedSince",
                        itemVersionModifiedSince.Value.ToUniversalTime());
            }))
            {
                while (reader.Read())
                {
            ...
            Guid[] ofTemplates = null, DateTime? modifiedSince = null, bool onlyPublishable = false)
        {
            ...
            using (var reader = sqlContext.ExecuteReader(sql, commandProcessor: cmd =>
            {
                ...
                if (onlyPublishable)
                {
                    cmd.Parameters.AddWithValue("@neverPublishFieldId", Sitecore.FieldIDs.NeverPublish.Guid);
        ...
        {
            var conn = new SqlConnection(ConfigurationManager.ConnectionStrings[database.ConnectionStringName]
                .ConnectionString);
            if (open) conn.Open();
            return conn;
        }
