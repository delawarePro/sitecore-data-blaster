...
namespace Sitecore.DataBlaster.Util
{
    public class CacheUtil
    {
        ...
        public virtual void RemoveItemsFromCachesInBulk(Database database,
            IEnumerable<Tuple<ID, ID, string>> itemIdParentIdAndItemPaths)
        {
            if (database == null) throw new ArgumentNullException(nameof(database));
        ...
        public virtual void ClearLanguageCache(Database database)
        {
            ...
            var field = typeof(SqlDataProvider).GetProperty("Languages",
            ...
            var cache = CacheManager.GetNamedInstance("LanguageProvider - Languages", Settings.Caching.SmallCacheSize,
                true);
