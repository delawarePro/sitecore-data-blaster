...
namespace Sitecore.DataBlaster.Load.Processors
{
    public class ChangeCacheClearer : IChangeProcessor
    {
        ...
        protected virtual IEnumerable<Tuple<ID, ID, string>> GetCacheClearEntries(IEnumerable<ItemChange> itemChanges)
        {
            ...
            var filteredItemChanges =
                itemChanges.Where(x => seenKeys.Add($"{x.ItemId}{x.ParentId}{x.OriginalParentId}{x.ItemPath}"));

            foreach (var itemChange in filteredItemChanges)
            {
                yield return new Tuple<ID, ID, string>(ID.Parse(itemChange.ItemId), ID.Parse(itemChange.ParentId),
                    itemChange.ItemPath);

                // Support moved items.
                if (itemChange.ParentId != itemChange.OriginalParentId)
                    yield return new Tuple<ID, ID, string>(ID.Parse(itemChange.ItemId),
                        ID.Parse(itemChange.OriginalParentId), itemChange.ItemPath);
            }
        }
    }
}